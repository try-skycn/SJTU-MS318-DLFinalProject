...
cmd:option('--saturateEpoch', 5000, 'epoch at which linear decayed LR will reach minLR')
...
cmd:option('--maxTries', 300, 'maximum number of epochs to try to find a better local minima for early-stopping')
...
cmd:option('--outputC', 64, "number of derived features for one sequence element")
cmd:option('--convkw', 3, 'number of sequence element kernel operates on per step')
cmd:option('--convdw', 1, 'step dw and go on to the next sequence element')
cmd:option('--poolkw', 2, 'number of sequence element pooled on per step when pooling')
cmd:option('--pooldw', 2, 'step dw and go on to the next sequence element when pooling')
...
cmd:option('--trainEpochSize', 1200, 'number of train examples seen between each epoch')
...
if opt.xpPath ~= '' then
	...
	net:add(nn.Linear(opt.outputC * nOutputFrame, 4))
